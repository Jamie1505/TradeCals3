...
const ArchitectStructuralLoadScreen= () => {
  const { addFavorite, removeFavorite, favorites } = useFavorites();

  // Check if the screen is a favorite
  const isFavorite = favorites.includes('ArchitectStructural');

  // Toggle favorite functionality
  const handleFavoritePress = () => {
    if (isFavorite) {
      removeFavorite('ArchitectStructural');
    } else {
      addFavorite('ArchitectStructural');
    }
  };
...
export default ArchitectStructuralLoadScreen;